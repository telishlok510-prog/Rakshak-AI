...
function CheckInner() {
  ...
  // Content shared in from another app (Android share sheet), e.g. a
  // WhatsApp/SMS message. Android's Web Share Target may put it in
  // "text" or "url" depending on what the source app shared.
  const sharedContent = params.get("text") || params.get("url") || undefined;

  const initial = (params.get("tab") as Tab) || "sms";
  // A share always lands on the SMS checker regardless of any ?tab= value,
  // since shared content is always plain text to analyze.
  const [tab, setTab] = useState<Tab>(
    sharedContent ? "sms" : TABS.some((x) => x.id === initial) ? initial : "sms"
  );
      ...
      <div className="card mt-6">
        {tab === "screenshot" ? (
          ...
          <TextChecker
            ...
            sample={SAMPLES[tab]}
            initialValue={tab === "sms" ? sharedContent : undefined}
          />
        )}
      </div>
    </div>
